...
<body>
 <div class="joincontainer">
 	<form method="post" action="../booking/training_list.do" class="search-container">
		<!-- <form method="post" action="../user/join_ok.do" name="join_frm" id="join_frm" > -->
			<h4>훈련 예약</h4>
			<!-- 동의사항 -->
			<div class="row">
				<div class="input-group">
						<input type="text" name=store id="search-bar" value="${store }" placeholder="훈련소 검색">
			...
			<h4>항목선택</h4>
			<div class="input-group" >
					<input type="checkbox" id="faaa" name="chitem" value="교육"> <label for="faaa" style="display: inline-block; text-align: center; margin-left: 20px">교육&nbsp;</label>
					<input type="checkbox" id="fbbb" name="chitem" value="유치원"> <label for="fbbb" style="display: inline-block; text-align: center; margin-left: 20px">유치원&nbsp;</label>
					<input type="checkbox" id="fccc" name="chitem" value="아카데미"> <label for="fccc" style="display: inline-block; text-align: center; margin-left: 20px">아카데미&nbsp;</label>
			</div>
	</div>
	</form>
</div>
  ...
  <%-- <div class="container" align="center">
		 <div class="pagination">
		  ...
		  <c:if test="${startPage>1 }">
		   <li><a href="../booking/training_list.do?store=${store }&page=${startPage-1 }&strChitem=${strChitem }">&laquo;</a></li>
		  </c:if>
			<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:if test="${i==curpage }">
		     <li class="active"><a href="../booking/training_list.do?store=${store }&page=${i }&strChitem=${strChitem }">${i }</a></li>
			</c:if>
			<c:if test="${i!=curpage }">
		    <li><a href="../booking/training_list.do?store=${store }&page=${i }&strChitem=${strChitem }">${i }</a></li>
			</c:if>
		    </c:forEach>
			<c:if test="${endPage<totalpage }">
			 <li><a href="../booking/training_list.do?store=${store }&page=${endPage+1 }&strChitem=${strChitem }">&raquo;</a></li>
			</c:if>
		  </ul>
		 </div>
		 </div> --%>
